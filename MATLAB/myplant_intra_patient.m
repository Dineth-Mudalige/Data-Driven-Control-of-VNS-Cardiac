% A function for the intra-patient variability model 
% 7-25-2021
% Andrew Branen

function [ini, HR, MAP] = myplant_inter_patient(ini, usim)
% Get the parameters 
load('par_greyid.mat', 'par_ini') 
par = struct(); 
...
function [par] = Get_Model_Constants(par, par_ini)
    ...
    par.stm.kw = 2.5e-4;
    par.stm.kf = 25;
    % This is what we changed. The influence of each location's fiber
    % recruitment percentage
    
    % 
    par.stm.loc = [1.1  0.1  0.2; ... % 0.8, 0.1, 0.4 Location 1 -> baroreceptive
                   0.1  0.8  0.0; ... % 0.1, 0.6, 0.1 Location 2 -> sympathetic
                   0.2  0.2  0.6];    % 0.1, 0.3, 0.3 Location 3 -> vagal 
    par.stm.fsp_as = 38;
    par.stm.fsp_es = 8;
    par.stm.fsp_ev = 8;
end
