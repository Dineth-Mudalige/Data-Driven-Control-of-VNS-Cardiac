...
Runs a closed-loop simulation of the minimum energy based controller design (section 2.2.2) with the physiological model. The results from this simulation are shown in Figure 4
...
def loss(y_sp, y, u):
    ...
    Loss = Sum(y_sp - y)^2 + Sum(u^2)
    
    Where the first sum represents a quadratic cost, and the second sum represents a L2 cost on the inputs.
    ...
    for i in range(t_steps):
        L2 += tf.square(u[:,i,:] + z_const)

    # Full cost is quadratic and L2, with L2 weight W 
    J = tf.reduce_sum((y_sp - y)**2) + W*tf.reduce_sum(L2)
    return J
...
def Loss(up, x, n, sp):
    """
    This is the same function as jacL1, however it will only return the loss value, instead of the gradient of the loss with respect to the inputs.
...
def controller(IC, Np, set_point, initial_guess):
    """
    This function specifies the controller by optimizing the VNS parameters
