...
Runs a closed-loop simulation of the offset-free controller design (section 2.4) with the overactive sympathetic nervous system modified physiological model. The results from this simulation are shown in Figure 10.
...
def Loss(up, x, n, sp, d):
   ...
   # make sure everything coming in is float32
    u1 = tf.cast((up), tf.float32) # = (8 + 6*Nc,)
...
####################################################################################
# state for diseased model
state_IC = [252.33551186292615, 2.7257842854338215, 114.40420071516408, 35.49896315904254, 0.19734287254257468, 0.047062944891714746, 0.11744037271491921, 0.14028861085294236, 0.1118017569444507, 1.700812075193425, 0.0]
...
# Disturbance gain
Ld = np.array([[0.06, 0.0], [0.0, 0.018]])
...
new_ini, HR_tot, MAP_tot = eng.myplant_sympathetic(state_IC, VNS_i, nargout=3)
...
for i in range(N):
    ...
    if soltag:
        ...
        new_ini, HR, MAP = eng.myplant_sympathetic(new_ini, VNS_action, nargout=3)
...
if plotflag:
    ...
    plt.subplot(121)
    plt.scatter(xts, ICs[:,0], c='purple')
    plt.plot([0, 300, 300, 600, 600, N], [356, 356, 393, 393, 377, 377], c='k')
    plt.ylabel('HR (bpm)')
    plt.xlabel('Cardiac Cycle')

    # Plots the blood pressure with cardiac cycle
    plt.subplot(122)
    plt.scatter(xts, ICs[:,1], c='purple')
    plt.plot([0,300,300,600,600,N], [150, 150, 129, 129, 143, 143], c='k')
    plt.ylabel('MAP (mmHg)')
    plt.xlabel('Cardiac Cycle')
    plt.show()
    ...
    # Plots the VNS Frequencies appiled by the controller
    plt.subplot(122)
