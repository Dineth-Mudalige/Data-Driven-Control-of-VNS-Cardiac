...
Runs a closed-loop simulation of the minimum overshoot based controller design (section 2.2.3) with the physiological model. The results from this simulation are shown in Figure 5.
...
##########################################################################################################
# loss function
def loss(y_sp, y, u, u_t1):
    """
    This function calculates the loss value using the following equation: 
    
    Loss = Sum(y_sp - y)^2 + Sum(|u|) + Sum(u* - u)
    
    Where the first sum represents a quadratic cost, the second sum represents a L1 cost on the inputs, and the third sum represents a penalty in changing inputs.
    ...
    # Each input has a different zeroing constant, effect of normalization
    ...
    # Delta u cost (second sum)
    delta_u = 0
    for i in range(t_steps):
        # select the prior u
        u_prev = u_t1 if i == 0 else u[:,i-1,:]
        delta_u += tf.abs(u[:,i,:] - u_prev)
        

    # Full cost
    J = tf.reduce_sum((y_sp - y)**2) + W*tf.reduce_sum(L1) + W2*tf.reduce_sum(delta_u)
    return J
    
##########################################################################################################    

# Gradient
def jac(up, x, n, sp, u_t1):
    """
    ...
    # the previously optimized inputs
    u_t1 = tf.constant([[u_t1]], dtype=tf.float32) 
...
def Loss(up, x, n, sp, u_t1):
    ...
    # and the previously selected input
    u_t1 = tf.constant([[u_t1]], dtype=tf.float32) 
...
##########################################################################################################
# Controller Function
def controller(IC, Np, set_point, initial_guess, ut1):
    """
    ...
    controller_actions = scipy.optimize.minimize(Loss, Nc*initial_guess,
                                         args=(IC, Np, set_point, ut1), method='SLSQP', bounds=Nc*bnds,
                                         jac=jac,
...
# Store the physiological variables of interest
ICs = np.zeros((151,2))

# Storing the VNS actions applied
...
# An initial guess, NOTE: this will be multiplied by Nc to give the correct dimensions
...
for i in range(150): 
    ...
    # Normalize the previously selected control actions 
    ut_1 = [0.,0.,0.,0.,0.,0.] if i == 0 else (applied_VNS - VNS_mean) / VNS_range
    
    # Run the controller to determine the optimal VNS parameters 
    fin_VNS = controller(controller_IC, Np, SP, guess_i, ut_1)
    
    # un normalize the optimized VNS paramters 
    fin_VNS = fin_VNS.reshape((Nc,6))*VNS_range + VNS_mean
    
    # store the applied action
